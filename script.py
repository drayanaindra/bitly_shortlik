...
def create_short_link():
    ...
    with open(origin_csv, 'rb') as f:
        for i, line in enumerate(f):
            if i > 0:
                raw_data = str(line, 'utf-8')
                
                get_school = raw_data.split(",")[0].replace(" ", "")
                get_origin_school = raw_data.split(",")[0]
                get_pic = raw_data.split(",")[1]
                get_city = raw_data.split(",")[2]
                get_classcode = raw_data.split(",")[3]
                get_originlink = raw_data.split(",")[4]
                cust_link = "bit.ly/"+"TOAKM_"+get_school
                ...
                if custom_short_link:
                    ...
                    short_link = res_to_json_shorlink.get("id")

                csv_data.append([get_origin_school, get_pic, get_city, get_classcode, get_originlink, short_link])
